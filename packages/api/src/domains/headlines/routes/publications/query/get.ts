...
export async function getPublicationsHandler(c: Context<AuthenticatedAppContext>) {
  const filters = await c.req.json().then(data => querySchema.parse(data));
  const publications = await getPublications({ database: c.env.DATABASE_URL }, filters);
